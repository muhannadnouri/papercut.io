...
- Rust stages a selected PDF under a 250 MB limit, then the existing lazy-loaded PDF.js path extracts and stores bounded page text, page locators, and a best-effort first-page thumbnail before atomically finalizing its SQLite FTS5 rows. PDFs are limited to 2,000 pages. Fully textless PDFs remain renderable but are marked **Text Recognition Required** instead of looking searchable. **Recognize English Text** runs the local Tesseract worker one page at a time, reports progress, supports cancellation, and exposes recognized text to search, Find, and TTS only after the existing atomic finalizer succeeds.
...
- Runtime import supports HTML, EPUB, and text-native PDF. Fully textless English PDFs remain renderable and are labeled **Text Recognition Required** until the user runs recognition. Hybrid page classification and additional languages remain deferred until they can be selected and validated without treating blank covers or separators as OCR failures.
