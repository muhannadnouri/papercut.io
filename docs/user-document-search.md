...
The current app supports local HTML, EPUB, and text-native PDF files:
...
- Tauri opens the native filesystem picker for `.html`/`.htm`, `.epub`, or `.pdf` files. Desktop folder import includes supported files directly inside the selected folder and skips subfolders.
...
- Rust stages a selected PDF under a 250 MB limit, then the existing lazy-loaded PDF.js path extracts and stores bounded page text, page locators, and a best-effort first-page thumbnail before atomically finalizing its SQLite FTS5 rows. PDFs are limited to 2,000 pages.
...
- Desktop builds can select one folder and feed up to 500 direct HTML, EPUB, or PDF children into that same batch pipeline. Folder traversal is non-recursive and does not recreate the filesystem folder in Papercut.
...
- PDF retains the original source because PDF.js needs it for rendering. Derived page text, FTS rows, and thumbnails can be rebuilt from that canonical source and are not embedded in portable audiobook bundles.
...
- Runtime import supports HTML, EPUB, and text-native PDF. Image-only/scanned PDFs still require the future OCR stage.
...
- EPUB files must be readable ZIP-based EPUB archives with a valid container and OPF spine, and must be at most 100 MB. Only local PNG, JPEG, GIF, and WebP images are retained, with 5 MB per image and 30 MB total reader-image caps. A declared EPUB 2/3 cover is retained beside the generated source under the same 5 MB per-image cap, and the Library gallery serves a persisted display-sized PNG thumbnail rather than decoding the original cover. Existing imports with retained covers receive that thumbnail lazily on first gallery access; imports without cover metadata are not retroactively reparsed.
- PDF files must have a valid PDF signature, be at most 250 MB and 2,000 pages, and expose extractable text for search/TTS. PDF.js rendering remains page-faithful and does not recolor page contents for app themes.
