...
- New imports retain the original filename as read-only provenance. **Document Info** lets users correct Papercut's display title without renaming the source, changing its stable URL, or invalidating folders, bookmarks, and saved audio.
