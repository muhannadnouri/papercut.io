...
- Search-result clicks can pass a lightweight reader target into `DocumentViewer`: a Pagefind heading hash, an uploaded reflowable-document section ordinal, and/or the first highlighted snippet text. HTML, EPUB, TXT, and Markdown readers map SQLite's persisted section ordinal to a generated `data-papercut-section` marker, then search that block before highlighting. Uploaded results with more than one body match use native `<details>` progressive disclosure for two additional ranked passages and a bounded twelve-bin occurrence strip. Each bin reports how many matching sections fall in that part of the document and opens the first supporting section/page in the bin. The reader keeps the clean document URL and avoids choosing an earlier duplicate phrase elsewhere in the book. Source-verified exact phrases retain a whole-reader fallback because the verified phrase may live outside SQLite's best broad candidate section. Reader text matching spans inline markup such as emphasis, links, and bold text, treats explicit line breaks as normalized whitespace, and does not cross readable block boundaries. The preferred visual marker is a named CSS Highlight range so large iOS/WebKit reader DOMs are not rewritten just to mark a search result.
...
- **Files** can select one or up to 500 HTML, EPUB, PDF, TXT, or Markdown files. A single successful import opens immediately; larger selections remain in the Library with count-based progress, the current filename, and localized semantic stages for format detection, reading, document/book preparation, storage, and PDF text extraction. Stage detail appears only when work lasts beyond a short delay; Papercut does not invent percentages for parser work without trustworthy units. Cooperative cancellation remains between files, successful siblings survive failures, and per-file errors remain expandable. Successful and cancelled import notices dismiss automatically after six seconds; failures remain available until dismissed.
...
5. SQLite groups matching sections by uploaded document before applying the result limit. It returns the best section, the complete matching-section count for each visible document, up to three ranked passages, and at most twelve occurrence bins computed from body matches. Each bin retains one source locator plus its exact matching-section count. Total matching-document/section counts are still computed before limiting the response.
...
8. Results share one panel but remain visibly separated into **Your Library** first and **Starter Documents** second. Each group states when only its initial 50 results are shown. Uploaded evidence stays collapsed behind the existing matching-section count until a user asks to explore it, avoiding three always-visible passages on every result card. Search phases use the existing polite live region; a reassurance that processing remains local appears only if the search lasts five seconds, while failures receive a distinct alert instead of being presented as **No results**.
9. Opening a result keeps the canonical document URL, then passes a best-effort target to the reader. Uploaded reflowable documents resolve their persisted SQLite section ordinal to a generated reader marker and constrain snippet-text matching to that indexed block; PDFs use their page target, while Pagefind starter documents keep heading-hash/text fallback behavior. Search-result navigation and in-document Find share block-aware text matching, so inline formatting does not split a visible phrase into separate matches. Starting an explicit Find retires the transient result highlight, caches the rendered reader's normalized text map, and uses CSS highlights without rewriting the reader DOM; audiobook playback highlights remain independent.
...
The current HTML, EPUB, PDF, TXT, and Markdown paths follow those rules. Uploaded search counts all lightweight matching section rows in Rust so one prolific document cannot consume the result limit, but sends at most 100 document cards over IPC. For visible documents, one additional body-only FTS scan selects at most three snippets and aggregates every matching section into twelve fixed position bins. The result payload and rendered evidence DOM are therefore bounded per document even for common terms in a long book. Broad terms do more local SQLite work than the previous best-snippet-only query, but search remains explicit-submit and evidence does not require loading document source. Progress reporting adds only small React state updates at existing async boundaries; the indexed-document count is memoized across library/scope changes, and the five-second detail is a CSS reveal rather than a polling timer. PDF extraction is sequential by page, search uses persisted SQLite page rows, and the viewer relies on PDF.js virtualization rather than mounting every rendered page eagerly.
...
- Multi-passage evidence and occurrence strips currently apply only to uploaded SQLite results. Each card retains at most three ranked passages and twelve position bins; a bin with several matches opens its first supporting section/page rather than listing every concordance line.
...
7. Add a document-by-query-term comparison matrix for selected uploaded results, reusing source-linked section/page evidence and keeping corpus vocabulary statistics out until multilingual tokenization is trustworthy.
