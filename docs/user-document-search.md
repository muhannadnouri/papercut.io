...
Keeping this shape stable lets the Library and SQLite indexing remain format-agnostic while each viewer loads its own stored source type. Translations reuse the same contract as derived document variants rather than mutating their source uploads. See [epub-implementation-plan.md](epub-implementation-plan.md) for the ordered EPUB task list and acceptance checks, [pdf-ocr-scanning.md](pdf-ocr-scanning.md) for the PDF, OCR, and mobile scanning plan, and [offline-translation.md](offline-translation.md) for translation architecture and status.
