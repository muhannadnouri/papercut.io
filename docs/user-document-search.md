...
7. Replace the generic search spinner with honest, scope-aware progress from the existing search phases: searching the selected library, loading ranked results, verifying exact phrases, and preparing excerpts. Announce phase changes accessibly, disclose the local large-library wait only after a delay, and do not invent percentages that the Pagefind/SQLite providers cannot measure.
8. Add a document-by-query-term comparison matrix for selected uploaded results, reusing source-linked section/page evidence and keeping corpus vocabulary statistics out until multilingual tokenization is trustworthy.
