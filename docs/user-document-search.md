...
- Users open **Import** from the document list and choose **Files** for one or more HTML, EPUB, PDF, TXT, or Markdown files, or **Folder** for a desktop folder. Desktop users can also drop supported files onto the open Library or choose Papercut from the operating system's **Open With** menu after installing the app.
...
- `src-tauri/src/desktop_open.rs` owns the removable desktop file-association edge: it filters OS launch requests, queues them across frontend startup or an active Library operation, and exposes one atomic drain command. It does not import documents itself.
...
- **Desktop Open With** uses installer-registered file associations for HTML, EPUB, PDF, TXT, and Markdown. Startup and second-instance requests enter a small native queue, focus the existing window, and wait for any active Library operation before entering the same path importer. macOS ranks Papercut as an alternate viewer; Windows and Linux use their standard handler registration. Raw, uninstalled AppImages depend on desktop integration before an operating system can list them as handlers.
