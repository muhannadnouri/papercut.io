...
are marked for future text recognition:
...
- Rust stages a selected PDF under a 250 MB limit, then the existing lazy-loaded PDF.js path extracts and stores bounded page text, page locators, and a best-effort first-page thumbnail before atomically finalizing its SQLite FTS5 rows. PDFs are limited to 2,000 pages. Fully textless PDFs remain renderable but are marked **Text Recognition Required** instead of looking searchable; OCR is not run yet.
...
- Library gallery and list views mark documents that have an explicit reader bookmark, and the Library toolbar can filter to those documents; bookmark creation and removal still happen only in the document viewer.
...
- Runtime import supports HTML, EPUB, and text-native PDF. Image-only/scanned PDFs remain renderable and are labeled **Text Recognition Required**, but they have no searchable or speakable text until the future OCR stage. Hybrid page classification is also deferred until it can distinguish image pages from blank covers and separators reliably.
