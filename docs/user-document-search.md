...
- `src/components/DocumentBrowser/bundledDocuments.ts` derives the read-only bundled-document hierarchy from canonical `/documents/...` URLs. Vite and Pagefind already preserve those paths, so bundled folders do not need a second manifest or SQLite organization records.
