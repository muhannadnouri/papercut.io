...
- Search-result clicks can pass a lightweight reader target into `DocumentViewer`: a Pagefind heading hash when available and/or the first highlighted snippet text. The reader keeps the clean document URL, then jumps to and marks the likely match after the document renders.
...
6. Quoted exact-phrase searches use Pagefind/SQLite only as a candidate finder, then verify the phrase against the real bundled or uploaded document source before rendering results.
7. Results are combined and rendered in the same panel.
8. Opening a result keeps the canonical document URL, then passes a best-effort target to the reader so Pagefind heading hashes or highlighted snippet text can scroll to the likely match.
9. If a result points to an uploaded document URL, source loading calls `document_uploads_get_source` instead of fetching from `dist/`.
...
For quoted searches, broad provider counts are not treated as exact phrase counts. The UI reports document-level phrase matches after source verification; unquoted searches can still show matching section counts when SQLite or Pagefind exposes them. Counting every exact occurrence or exact section is a possible later improvement, but it should be based on the same normalized source text used for verification.
...
- Quoted exact-phrase results are document-level matches. Exact occurrence counts or exact section counts would need a separate source-based counter.
