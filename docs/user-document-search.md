...
9. Opening a result keeps the canonical document URL, then passes a best-effort target to the reader so Pagefind heading hashes or highlighted snippet text can scroll to the likely match. Search-result highlighting uses its own named CSS Highlight entry and does not clear audiobook playback highlights.
