...
7. Build the first Search Evidence Explorer slice from existing SQLite metadata: multiple passages and a compact query-occurrence strip per uploaded document, with every passage/marker opening through the new section target and without a chart dependency.
