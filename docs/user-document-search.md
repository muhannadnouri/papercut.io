...
- Desktop builds can select one folder and feed up to 500 HTML, EPUB, or PDF files into that same batch pipeline. Papercut recreates the selected folder and supported descendants through five visible Library levels, skips symlinks and deeper descendants, and gives a conflicting top-level folder a numeric suffix instead of merging it. Exact duplicate documents already in the Library remain in their current location.
