...
- Users open **Import** from the document list and choose **Files** for one or more HTML, EPUB, PDF, TXT, or Markdown files, or **Folder** for a desktop folder. Desktop users can also drop supported files onto the open Library.
...
1. **Choose source**: a native Tauri dialog selects one file, multiple files, or one desktop folder; desktop users can instead drop files onto the Library. Source selection stays separate from the per-file importer so every entry point reuses the same validation and persistence path.
