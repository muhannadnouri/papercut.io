...
- Pagefind and SQLite FTS are still used first to find likely phrase candidates, then the app reads the applicable source, normalizes whitespace and curly quotes, lowercases, and checks each quoted phrase as a substring. The Pagefind `content` field and SQLite FTS section counts are not treated as exact phrase counts because they come from broad candidate matching. Source-verified occurrence counts are shown where available; unquoted searches can show matching-section counts when the provider exposes them. Source reads are cached per URL in memory for the session to avoid re-fetching across queries.
