...
- Uploaded-document snippets are produced by SQLite FTS and sanitized again before rendering in React. SQLite groups sections before applying the document limit, returns the best matching snippet per uploaded document, and reports complete matching-document/section counts before limiting the visible cards. Results with evidence in multiple sections can be expanded to show two more ranked passages and a twelve-bin occurrence strip; counts inside a bin show locally concentrated matches, and every passage/bin opens its supporting section or PDF page. Broad searches containing two to six terms also let users select up to five uploaded results for a document-by-term comparison table. Each cell reports the exact number of matching sections for that term and opens its first supporting section or PDF page. The evidence stays collapsed until requested so the result list remains easy to scan. HTML, EPUB, TXT, and Markdown use persisted section ordinals and safe reader markers, while PDFs keep page-aware targets.
