...
function getAiAudioExportDescription(record: SavedAudiobookRecord): string {
  const description = 'This export contains synthetic speech generated by Papercut. If you share, publish, sell, or broadcast it, label it as AI-generated and do not use it to impersonate or mislead.'
  if (record.modelId !== SILMA_MODEL_ID) return description
  return description + ' This voice uses a reference recording, so only export it if you have permission to use that voice and recording.'
}

function getAiAudioExportDisclosure(record: SavedAudiobookRecord): string {
  if (record.modelId === SILMA_MODEL_ID) return 'AI-generated audio; reference voice permissions required'
  return 'AI-generated audio; label if shared'
}
...
export function useAudiobookManager({
  ...
  const handleExportSavedAudiobook = useCallback(async (record: SavedAudiobookRecord, exportFormat: NativeAudiobookExportFormat) => {
    const confirmed = await confirmAudiobookAction({
      title: 'Export AI-generated audiobook?',
      ...
      details: [
        ...
        { label: 'Disclosure', value: getAiAudioExportDisclosure(record) },
      ],
      confirmLabel: 'Export',
    })
    if (!confirmed) return
